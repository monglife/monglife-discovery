...
interface Props {
  ...
  disabled?: boolean;
  loading?: boolean;
  onSubmit: (values: { isSleep: boolean; reason?: string }) => void;
}
...
export function MongSleepForm({ isSleep, disabled, loading, onSubmit }: Props) {
  ...
  return (
    <div className="space-y-3">
      <Field label="현재" hint={isSleep ? '증가 스케줄이 돌고 있다' : '감소·배변 스케줄이 돌고 있다'}>
      ...
      <Field label="사유" hint="로그에 남는다.">
        <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="예: 스케줄이 꼬여 수면 상태가 틀어짐" />
      </Field>
      <div className="flex justify-end">
        <Button
          ...
          disabled={disabled}
          loading={loading}
          onClick={() => { onSubmit({ isSleep: next, reason: reason || undefined }); setReason(''); }}
        >
